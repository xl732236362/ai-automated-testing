...
- [x] Add tests proving `parse_decision` preserves `detected_controls`, `detected_cursors`, `detected_targets`, and `control_hypothesis`, and older decisions default those fields to empty collections.
- [x] Add tests proving `_build_decision_prompt` asks for visual anchors when direct actions have failed or gesture switching is recommended.
- [x] Add normalizers for visual anchors and control hypotheses.
- [x] Extend `_decision_schema()` with optional visual-anchor fields.
- [x] Run `python -m unittest tests.test_game_reverse_llm_decider`.
- [x] Commit with `feat: add visual anchor decision schema`.
...
- [x] Add failing tests for validating `aim_fire` anchors, rejecting malformed anchors, and still rejecting raw `touch_*` public actions.
- [x] Add `enable_continuous_actions` to `GameReverseConfig` with default `False`.
- [x] Add `aim_fire` to public safe-action names but require it to appear in `allowed_actions`.
- [x] Add Web/API validation: `aim_fire` requires `enable_unsafe_actions=true` and `enable_continuous_actions=true`.
- [x] Add Web UI opt-in for continuous gestures; include `aim_fire` only when both unsafe and continuous toggles are on.
- [x] Run `python -m unittest tests.test_game_reverse_actions tests.test_game_reverse_config tests.test_game_reverse_web_service tests.test_web_console_static`.
- [x] Commit with `feat: gate aim fire continuous action`.
...
- [x] Add failing tests for control screenshot naming and `control_sessions.jsonl` writes.
- [x] Add failing tests for `AimController.execute()` calling `touch_down`, `touch_hold`, bounded `touch_move`, `touch_up`, and cleanup on errors.
- [x] Implement `Journal.control_screen_path()` and `Journal.write_control_event()`.
- [x] Implement `AimController` with bounded geometry movement and internal pointer commands.
- [x] Run `python -m unittest tests.test_game_reverse_continuous_control tests.test_game_reverse_journal`.
- [x] Commit with `feat: add aim controller control sessions`.
...
- [x] Add failing integration test where a decider emits `aim_fire`, a fake executor records internal pointer commands, and the run loop writes action, observation, feedback, and control-session artifacts.
- [x] Route `aim_fire` through `AimController` instead of direct `executor.execute`.
- [x] Include visual-anchor fields in observations.
- [x] Attach `control_feedback` to action and feedback artifacts when available.
- [x] Ensure executor cleanup runs before finalization if a continuous action errors.
- [x] Run `python -m unittest tests.test_game_reverse_run_loop`.
- [x] Commit with `feat: execute aim fire in run loop`.
...
- [x] Add tests mapping `counter_changed` to `target_collected` for continuous actions and repeated no-change to `control_attempt_failed`.
- [x] Add tests mining successful `aim_fire` actions into `continuous_control` parameterized skills.
- [x] Add memory summary coverage for continuous-control skills.
- [x] Implement feedback mapping and skill mining.
- [x] Run `python -m unittest tests.test_game_reverse_feedback tests.test_game_reverse_skill_library tests.test_game_reverse_profile_learning`.
- [x] Commit with `feat: learn continuous control skills`.
...
- [x] Add tests proving only registered single-pointer controllers are available by default.
- [x] Add a small `ContinuousControllerRegistry` with `AimController` registered and multi-touch disabled.
- [x] Ensure future controller names are rejected with a clear error.
- [x] Run `python -m unittest tests.test_game_reverse_continuous_control`.
- [x] Commit with `feat: add continuous controller registry`.
...
- [x] Restart the Web service with `.\restart-web.bat`.
